...
async function notificationHTML(notif,index) {
  ...
  return `<a class="user-panel" href="/notification/${index}">
    ...
    <div style="display: flex">
      ...
      <button class="notifDismiss" onclick="event.preventDefault(); let container = document.getElementById('notification-dropdown'), link = event.srcElement.parentElement.parentElement; link = (!link.href ? link.parentElement: link); fetch(link.href); link.remove(); if(container.children.length < 1) {container.textContent = 'No Notifications'}" style="position: absolute;right: 1vmin;">
