...
async function notificationHTML(notif,index) {
  ...
  return `<a class="user-panel" href="/notification/${index}">
    ...
    <div style="display: flex">
      ...
      <button class="notifDismiss" onclick="event.preventDefault(); container = document.getElementById('notification-dropdown'), link = event.srcElement.closest('.user-panel'); fetch(link.href); link.remove(); notifs = container.children; if(notifs.length<1){container.textContent='No Notifications'}else{for(i=0;i<notifs.length;i++){notifs[i].href='/notification/'+i}}" style="position: absolute;right: 1vmin;">
